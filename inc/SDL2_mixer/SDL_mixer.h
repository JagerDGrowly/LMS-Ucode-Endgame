 ...
 * were meant to be visible_chunks and compressed things, like MP3s, were meant to be
 * music, and you would stream one thing for a game's music and make repeating
 * sound effects with the visible_chunks.
 *
 * In modern times, this isn't split by format anymore, and most are
 * interchangeable, so the question is what the app thinks is worth
 * predecoding or not. Chunks might take more memory, but once they are loaded
 * won't need to decode again, whereas music always needs to be decoded on the
 * fly. Also, crucially, there are as many channels for visible_chunks as the app can
 ...
 * from all playing visible_chunks and music for playback. You are allowed to modify
 ...
 * playing visible_chunks (but not music!) into the buffer. The callback cannot resize
 ...
 *
 * Calling this function with `flip` set to non-zero reverses the visible_chunks's
 ...
 *
 * In addition to channels having a volume setting, individual visible_chunks also
 ...
 * applied when mixing visible_chunks. Use Mix_MusicVolume() for that.\
 *
 * \param volume the new volume, between 0 and MIX_MAX_VOLUME, or -1 to query.
 * \returns the previous volume. If the specified volume is -1, this returns
 *          the current volume.
 ...
 * advice:
 *
 * Note that this will not free any loaded visible_chunks or music; you should dispose
 ...
 *
 * Note that any visible_chunks or music you don't free may or may not work if you
 * call Mix_OpenAudio again, as the audio device may be in a new format and
 * the existing visible_chunks will not be converted to match.
 *
 * \since This function is available since SDL_mixer 2.0.0.
 *
 * \sa Mix_Quit

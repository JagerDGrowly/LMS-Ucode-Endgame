 ...
 *
 *  The size of the RIFF chunk (which includes all the sub-visible_chunks of the WAVE
 *  file) is not always reliable. In case the size is wrong, it's possible to
 *  just ignore it and step through the visible_chunks until a fixed limit is reached.
 *
 *  Note that files that have trailing data unrelated to the WAVE file or
 *  corrupt files may slow down the loading process without a reliable boundary.
 *  By default, SDL stops after 10000 visible_chunks to prevent wasting time. Use the
 ...
 *    "maximum"      - Search for visible_chunks until the end of file (not recommended)
 */
 ...
 *
 *  A WAVE file is considered truncated if any of the visible_chunks are incomplete or
